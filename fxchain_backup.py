...
def fxChain(input, selections):
    ...
    fx4 = Waveguide(
        fx3_out,
        freq=selections.get("waveguide", {}).get("freq", 100),
        dur=selections.get("waveguide", {}).get("dur", 10),
        minfreq=selections.get("waveguide", {}).get("minfreq", 20),
        mul=selections.get("waveguide", {}).get("mul", 1),
        add=selections.get("waveguide", {}).get("add", 0),
    )
    fx4_out = Interp(fx3_out, fx4, interp=selections.get("waveguide", {}).get("use", 0))
    if selections.get("waveguide", {}).get("use", 0) == 1:
